...
- **Contexts:** (Deprecated: Use standard aggregates with a constant Id instead) Event-sourced objects with no ID, where only one instance is available per type. They define an initial state using a static `Zero` member.
...
- **`Core.fs`:** Defines `Events`, `Commands`, and pure `evolve` functions defining how events advance the state of aggregates and contexts.
...
- **`PgEventStore.fs` / `MemoryStorage.fs`:** PostgreSQL and in-memory event stores using text/JSON format.
- **`PgBinaryEventStore.fs`:** PostgreSQL event store using binary serialization (via external libraries like FsPickler).
